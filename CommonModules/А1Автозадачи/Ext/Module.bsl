...
#Если НЕ Клиент Тогда
	...
	Функция НовыйНастройка() Экспорт 
		...
		"ТекстЗапроса", "",
		"ПараметрыЗапроса", , //Структура или Строка. Если строка - воспринимается как функция, которая должна вернуть структуру параметров.
	...
	Функция ВсеНастройки(ПовтИсп = Истина) Экспорт
	...
	Функция ПроверитьИДополнитьНастройки(НастройкиАвтозадач) Экспорт 
		Для Каждого Настройка Из НастройкиАвтозадач Цикл
	...
	Функция ТекстЗапросаАнализаЗадач(ТолькоИзмененные) Экспорт
		...
		|ГДЕ
		|	(АктуальныеЗадачи.Источник = &ИсточникИдентификатор
	...
	Функция ДополнитьТаблицуАктуальныхЗадач(АктуальныеЗадачи, Настройка) Экспорт
		...
		КонецЕсли;
		Если АктуальныеЗадачи.Колонки.Найти("Источник") <> Неопределено Тогда
			АктуальныеЗадачи.Колонки.Источник.Имя = "А1Автозадачи_ИсточникЗапроса";
		Иначе
			АктуальныеЗадачи.Колонки.Добавить("Источник", А1Э_Строки.ОписаниеТипа(50));
		КонецЕсли;
