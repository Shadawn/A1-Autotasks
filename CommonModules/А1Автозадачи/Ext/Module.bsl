...
#Если НЕ Клиент Тогда
	...
	Функция НовыйНастройка() Экспорт 
		...
		"ИспользоватьЗаместителей", Ложь //Булево,Строка,Массив - если Истина, то колонка "Исполнитель" финального запроса поддерживает механизм заместителей. Если строка, то это список реквизитов, которые будут пропущены сквозь механизм заместителей.
		) 
	КонецФункции
	...
	Функция Обновить(ТолькоИзмененные = Истина, Знач Источник = Неопределено) Экспорт
		...
		Для Каждого Настройка Из НастройкиАвтозадач Цикл
			...
			Если Настройка.ИспользоватьЗаместителей <> Ложь Тогда
				Если Заместители = Неопределено Тогда
					Заместители = АктивныеЗаместители();
					Если Заместители = Ложь Тогда
						ВызватьИсключение "Обнаружены циклы в назначенных заместителях!";
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			
			ДополнитьТаблицуАктуальныхЗадач(АктуальныеЗадачи, Настройка, Заместители);
	...
	Функция АктивныеЗаместители() Экспорт
		Заместители = Новый Соответствие;
		
		Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Заместители.ОтветственноеЛицо КАК ОтветственноеЛицо,
		|	Заместители.Заместитель КАК Заместитель
		|ИЗ
		|	РегистрСведений.А1Автозадачи_Заместители КАК Заместители");
		Выборка = Запрос.Выполнить().Выбрать();
		
		//Обходим все пары заместителей и создаем цепочки Ответственный -> Заместитель -> Его заместитель -> ...
		//Для каждого не последнего элемента цепочки окончательным заместителем будет последний элемент.
		НачалаЦепочек = Новый Соответствие;
		КонцыЦепочек = Новый Соответствие;
		Пока Выборка.Следующий() Цикл
			ЦепочкаПоКонцу = КонцыЦепочек[Выборка.ОтветственноеЛицо];
			ЦепочкаПоНачалу = НачалаЦепочек[Выборка.Заместитель];
			Если ЦепочкаПоКонцу <> Неопределено Тогда
				Если ЦепочкаПоНачалу <> Неопределено Тогда
					Если ЦепочкаПоКонцу = ЦепочкаПоНачалу Тогда
						Сообщить("Обнаружен цикл в назначенных заместителях! В цикле участвуют пользователи " + СтрСоединить(ЦепочкаПоКонцу, ", "));
						Возврат Ложь;
					КонецЕсли;
					
					ОбщаяЦепочка = А1Э_Массивы.Добавить(ЦепочкаПоКонцу, ЦепочкаПоНачалу);
					//Эта проверка скорее всего не нужна, но я оставлю её на всякий случай.
					Если ОбщаяЦепочка[0] = А1Э_Массивы.Последний(ОбщаяЦепочка) Тогда
						Сообщить("Обнаружены циклы в назначенных заместителях! Проверьте заместителей пользователя " + ОбщаяЦепочка[0]);
						Возврат Ложь;
					КонецЕсли;
					КонцыЦепочек.Удалить(Выборка.ОтветственноеЛицо);
					НачалаЦепочек.Удалить(Выборка.Заместитель);
					КонцыЦепочек.Вставить(А1Э_Массивы.Последний(ОбщаяЦепочка), ОбщаяЦепочка);
					НачалаЦепочек.Вставить(ОбщаяЦепочка[0], ОбщаяЦепочка);
				Иначе
					ЦепочкаПоКонцу.Добавить(Выборка.Заместитель);
					КонцыЦепочек.Удалить(Выборка.ОтветственноеЛицо);
					КонцыЦепочек.Вставить(Выборка.Заместитель, ЦепочкаПоКонцу);
				КонецЕсли;
			ИначеЕсли ЦепочкаПоНачалу <> Неопределено Тогда
				ЦепочкаПоНачалу.Вставить(0, Выборка.ОтветственноеЛицо);
				НачалаЦепочек.Удалить(Выборка.Заместитель);
				НачалаЦепочек.Вставить(Выборка.ОтветственноеЛицо, ЦепочкаПоНачалу);		
			Иначе
				НоваяЦепочка = Новый Массив;
				НоваяЦепочка.Добавить(Выборка.ОтветственноеЛицо);
				НоваяЦепочка.Добавить(Выборка.Заместитель);
				НачалаЦепочек.Вставить(Выборка.ОтветственноеЛицо, НоваяЦепочка);
				КонцыЦепочек.Вставить(Выборка.Заместитель, НоваяЦепочка);
			КонецЕсли;
		КонецЦикла;
		Для Каждого Пара Из КонцыЦепочек Цикл
			Для Каждого Элемент Из Пара.Значение Цикл
				Если Элемент <> Пара.Ключ Тогда
					Заместители.Вставить(Элемент, Пара.Ключ);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
...
#Если НЕ Клиент Тогда
	...
	Функция ДополнитьТаблицуАктуальныхЗадач(АктуальныеЗадачи, Настройка, Заместители)
		...
		КонецЕсли;
		Если Настройка.ИспользоватьЗаместителей <> Ложь Тогда
			Если Настройка.ИспользоватьЗаместителей = Истина Тогда
				МассивПолей = А1Э_Массивы.Создать("Исполнитель");
			Иначе
				МассивПолей = А1Э_Массивы.Массив(Настройка.ИспользоватьЗаместителей);
			КонецЕсли;
			Для Каждого Строка Из АктуальныеЗадачи Цикл
				Для Каждого Поле Из МассивПолей Цикл
					Заместитель = Заместители[Строка[Поле]];
					Если Заместитель <> Неопределено Тогда
						Строка[Поле] = Заместитель;
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
		КонецЕсли;
	...
	Функция СоздатьЗадачу(ДанныеЗадачи, ВсеДанные, ИмяМетаданных)
		...
		ОбъектЗадача.ДополнительныеСвойства.Вставить("А1Автозадачи", А1Э_Структуры.Создать(
		"Данные", ВсеДанные,
		));
